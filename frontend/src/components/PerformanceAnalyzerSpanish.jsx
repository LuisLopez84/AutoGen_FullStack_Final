...
const renderizarResumen = () => {
      ...
      <div className="grid-puntuaciones">
        {Object.values(categories || {}).map((categoria) => {
          ...
          return (
            <div
              key={categoria.id}
              className="tarjeta-puntuacion"
              style={{ borderColor: color }}
            >
              <div className="categoria">{categoria.title}</div>
              <div className="puntuacion" style={{ color }}>
                {categoria.score}
              </div>
  ...
  const exportarReporteCompleto = async (formato) => {
    if (!url.trim()) {
      setError("Ingresa una URL primero para exportar");
      return;
    }

    // Verificar que tenemos resultados
    if (!resultados) {
      setError("Primero debes analizar la URL para generar el reporte");
      return;
    }
